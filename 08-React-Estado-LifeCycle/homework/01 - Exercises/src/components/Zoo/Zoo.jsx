import React from "react";
// eslint-disable-next-line no-unused-vars
import Animals from "../Animals/Animals";
// eslint-disable-next-line no-unused-vars
import Species from "../Species/Species";
// import styledZoo from "./Zoo.module.css";

export default function Zoo() {
  /* Escribe acá tu código */
  const [zoo, setZoo] = React.useState({
    zooName: '', 
    animals: [], 
    species: [], 
    allAnimals: [],
  });

  const handleInputChange = (e) => {
    setZoo({
      ...zoo, //traigo a todos los elementos del zoo
      zooName: e.target.value // con esto solo modifico el zooName ya q traje a todos con ...zoo
    });
  }

  React.useEffect(()=>{
    fetch('http://localhost:3001/zoo')
   .then((res) => res.json())
   .then((data) =>

      setZoo({
         ...zoo,
         animals: data.animals,
         species: data.species,
         allAnimals: data.animals,
      }) 
   )
   .catch((error) => console.log(error));
  },
  [])

  const handleSpecies = (e) => {
    const species = e.target.value
    setZoo({
      ...zoo,
      animals: zoo.allAnimals.filter((animal)=>{
        return animal === animal.specie
      })
    })}

  const handleAllSpecies = () => {
    setZoo({
      ...zoo,
      animals: zoo.allAnimals
    })
  }



  return (
    <div>
      <label htmlFor="">Zoo Name:</label>
      <input type="text" value={zoo.zooName} onChange={handleInputChange}/>
      <h1>{zoo.zooName}</h1>
      <Species species={zoo.species} handleSpecies={handleSpecies} handleAllSpecies={handleAllSpecies}/>
      <Animals animals={zoo.animals}/>
    </div>
  );
}















// const fetch = async () =>{
//   try{
//     const response = await fetch('http://localhost:3001/zoo')
//     const data = await response.json();
    
//     setZoo({
//       ...zoo,
//       animals: data.animals,
//       species: data.species,
//       allAnimals: data.animals,
//     })
// }
// catch (error){
//   console.log(error)
// }
// }